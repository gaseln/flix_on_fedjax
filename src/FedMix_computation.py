...
def evaluate_model(
    ...
    for alpha, plm, cds in client_data:
        personalized_params = FedMix.convex_combination(params, plm, alpha)
...
class FedMixHParams:
    ...
    Attributes:
        lr: A learning rate of an algorithm
...
class FedMixComputationParams:
    ...
    Attributes:
        optimizer: A server-lever optimizer.
...
def fedmix_computation_with_statistics(
    ...
    if fedmix_comp_params.optimizer == 'adam':
        server_optimizer = fedjax.optimizers.adam(
            learning_rate=fedmix_hparams.lr, b1=0.9, b2=0.999, eps=10**(-4)
        )
    elif fedmix_comp_params.optimizer == 'sgd':
        server_optimizer = fedjax.optimizers.sgd(
            learning_rate=fedmix_hparams.lr
        )
    else:
        raise ValueError(
    ...
    client_batch_hparams = fedjax.ShuffleRepeatBatchHParams(
        batch_size=fedmix_hparams.client_batch_size, num_steps=1)
    algorithm = FedMix.fedmix(
        grad_fn, server_optimizer, client_batch_hparams, plms, alphas)
